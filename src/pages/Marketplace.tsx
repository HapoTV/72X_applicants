...
const Marketplace: React.FC = () => {
  ...
  const [storageAuthToken, setStorageAuthToken] = useState<string | null>(
  ...
  const isOwnPreviewProduct =
    Boolean(currentUserId) &&
    String(previewProduct?.sellerId || '') === currentUserId;

  /*
   * Fetch the current user's products.
   * IMPORTANT: This callback is declared before the useEffect that uses it.
   */
  const fetchMyProducts = useCallback(async () => {
    ...
    try {
      ...
      setMyProductsError(
        e?.message || 'Failed to fetch my products'
      );
   ...
   * IMPORTANT: This callback is declared before the useEffects and
  ...
  const fetchMarketplaceData = useCallback(async () => {
    ...
    try {
      ...
      try {
        if (activeView === 'featured') {
          ...
          const featuredActive = Array.isArray(featured)
            ? featured.filter((p) => p?.status !== 'sold')
            : [];
          ...
          if (featuredActive.length > 0) {
            ...
            const productsResponse =
              await marketplaceService.getActiveProducts({});
            ...
            if (derived.length > 0) {
              ...
              const cachedSafe = Array.isArray(cached)
                ? cached.filter((p) => p?.status !== 'sold')
                : [];
          ...
          if (featuredSearchTerm) {
            searchParams.query = featuredSearchTerm;
          }

          if (featuredSelectedCategory !== 'all') {
            searchParams.category = featuredSelectedCategory;
          }

          if (featuredSelectedLocation !== 'all') {
            searchParams.location = featuredSelectedLocation;
          }

          const productsResponse =
            await marketplaceService.getActiveProducts(searchParams);
        ...
        /*
         * Retry Marketplace request once.
         */
        try {
          if (activeView === 'featured') {
            const featured =
       ...
       * If those endpoints are unavailable, use the existing fallback data.
       */
      try {
        ...
        console.log(
          'Categories/Locations API not available, using fallback data'
        );
    ...
    activeView,
    featuredSearchTerm,
    featuredSelectedCategory,
    featuredSelectedLocation,
    storageAuthToken,
    token
  ]);
  ...
  const openProductPreview = async (product: UserProductItem) => {
    try {
      ...
      const details =
        await marketplaceService.getProductById(product.id);
      ...
      setPreviewError(
        'Could not load full product details. Showing a basic preview.'
      );
    } finally {
  ...
  const handleContactSellerFromPreview = () => {
    ...
    const title =
      typeof previewProduct?.title === 'string'
        ? previewProduct.title
        : 'this product';

    const message =
      `Hello, I would like to purchase the product: ${title}.`;
    ...
    navigate(
      `/community?tab=connections&${params.toString()}`
    );
  ...
  const handleListProduct = async (e: React.FormEvent) => {
    ...
    if (isFreeTrialUser) {
      alert(
        'Users on Free Trial cannot list products. Please subscribe to a plan to continue.'
      );
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          'Product title, description, and price are required'
        );
      }

      if (!productData.category || !productData.location) {
        throw new Error(
          'Product category and location are required'
        );
      }
  ...
  /*
   * Fetch Marketplace data when the active view/auth/filter state changes.
   */
  useEffect(() => {
  ...
  /*
   * Reset featured filters when returning to the featured view.
   */
  useEffect(() => {
    if (activeView !== 'featured') return;
  ...
  const handleDeleteMyProduct = async (
    ...
    const confirmed = window.confirm(
      'Delete this product? This will remove it for all users.'
    );
  ...
  const handleToggleMyProductStatus = async (
    ...
    try {
      ...
      await marketplaceService.updateProductStatus(
        product.id,
        nextStatus
      );
  ...
  const filteredProducts = products.filter(
    (product) => {
      ...
      const search =
        featuredSearchTerm.toLowerCase();
      ...
      const matchesCategory =
        featuredSelectedCategory === 'all' ||
        product.category ===
          featuredSelectedCategory;

      const matchesLocation =
        featuredSelectedLocation === 'all' ||
        location.includes(
          featuredSelectedLocation.toLowerCase()
        );
  ...
  const filteredMyProducts = myProducts.filter(
    (product) => {
      ...
      const matchesLocation =
        mySelectedLocation === 'all' ||
        location.includes(
          mySelectedLocation.toLowerCase()
        );
