...
const MarketingSales: React.FC = () => {
  ...
  const totalTimeSpent = 135; // minutes (60% of 2h 15min)
  const learningStreak = 5; // days
  const certificatesEarned = 0;
  const lastAccessed = 'Yesterday';

  // Original modules from LearningModules
  const allModules = [
    {
      id: '1',
  ...
  // Filter to show only marketing category
  const modules = allModules.filter(m => m.category === 'marketing');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Marketing & Sales</h1>
        <p className="text-gray-600">Learn how to attract customers and grow your revenue</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-3xl font-bold text-green-600">{modules.filter(m => m.progress === 100).length}</div>
            <div className="text-sm text-gray-600 mt-1">Completed</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-orange-600">{modules.filter(m => m.progress > 0 && m.progress < 100).length}</div>
            <div className="text-sm text-gray-600 mt-1">In Progress</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-gray-600">{modules.filter(m => m.progress === 0).length}</div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-medium text-gray-900">{Math.round(modules.reduce((sum, m) => sum + m.progress, 0) / modules.length)}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div className="bg-primary-500 h-3 rounded-full transition-all duration-300" style={{ width: `${Math.round(modules.reduce((sum, m) => sum + m.progress, 0) / modules.length)}%` }} />
          </div>
        </div>
