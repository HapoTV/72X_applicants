// src/pages/adminDashboard/tabs/components/OrgAdminBusinessRefPanelView.tsx
import React from 'react';
import { KeyRound, Eye, EyeOff, RefreshCw, Save } from 'lucide-react';
import type { OrganisationDTO } from '../../../../services/OrganisationService';

export type OrgAdminBusinessRefPanelViewProps = {
  org: OrganisationDTO;
  success: string;
  error: string;
  showRef: boolean;
  ...
  newRef: string;
  saving: boolean;
  onToggleShowRef: () => void;
  onRefresh: () => void;
  onStartEdit: () => void;
  onCancelEdit: () => void;
  onNewRefChange: (value: string) => void;
  onSave: () => void;
};

export const OrgAdminBusinessRefPanelView: React.FC<OrgAdminBusinessRefPanelViewProps> = ({
  org,
  success,
  error,
  showRef,
      ...
      {!editing ? (
        ...
        <div className="border-t border-gray-100 pt-4 space-y-3">
          ...
          <input
            type="text"
            value={newRef}
            onChange={e => onNewRefChange(e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent font-mono text-sm"
            placeholder="Enter new reference..."
            autoFocus
          />
          <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded p-2">
            ⚠️ Changing this will silently update all employee accounts. The old reference will no longer work for new
            signups.
          </p>
