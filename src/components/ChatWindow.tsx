...
const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  useEffect(() => {
    ...
    if (!isOpen || !shouldAutoSend || !content || !currentUserId || !receiverId) return;
  ...
  const buildAttachmentSummary = () => {
    ...
    const attachmentNames = selectedAttachments.map(
      ({ file, category }) => `${category}: ${file.name}`
    );
  ...
  const getReadReceipt = (message: Message, isOwnMessage: boolean) => {
    ...
    return (
      <Box
        ...
        sx={{
          ml: 0.5,
          color: message.isRead ? '#64b5f6' : 'rgba(255,255,255,0.7)',
          fontWeight: 700,
        }}
      >
        ✓✓
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        {getReadReceipt(message, isOwnMessage)}
                      </Typography>
                    </Paper>
                  </Box>
                </ListItem>
