...
async analyzeUploadedFile(request: FileAnalysisRequest): Promise<AIAnalysisResponse> {
  try {
    ...
    if (request.fileUrl) formData.append('fileUrl', request.fileUrl);
    if (request.analysisType) formData.append('analysisType', request.analysisType);
    if (request.query) formData.append('query', request.query);
    
    const response = await axiosClient.post(`${this.baseURL}/analyze/file`, formData);
...
async getAvailableDocuments(): Promise<AvailableDocument[]> {
  try {
    const response = await axiosClient.get(`${this.baseURL}/documents/available`);
  ...
  // Add this method to poll for analysis completion
async getAnalysisWithPolling(analysisId: string): Promise<AIAnalysisResponse> {
  try {
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      ...
      if (statusResponse.status === 'completed') {
        ...
        const response = await axiosClient.get(`${this.baseURL}/analysis/${analysisId}`);
        const data = response.data;
...
// Add this method to get analysis status
async getAnalysisStatus(analysisId: string): Promise<{ status: string }> {
  try {
    const response = await axiosClient.get(`${this.baseURL}/analysis/${analysisId}/status`);
    return {
...
// Update the submitAnalysis method to use polling
async submitAnalysis(request: AIAnalysisRequest): Promise<AIAnalysisResponse> {
  try {
    ...
    return await this.getAnalysisWithPolling(data.analysisId);
